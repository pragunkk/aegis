...
export const MetricsBar: React.FC<MetricsBarProps> = ({ logs }) => {
  ...
  const paymentsConfirmed = logs.filter((l) => l.event_type === 'PAYMENT_CONFIRMED');

  const totalSettledRupees = paymentsConfirmed.reduce((sum, item) => {
    const amt = item.payload?.negotiated_price || (item.payload?.amount ? item.payload.amount / 100 : 0);
  ...
  const stats = [
    {
      ...
      subtitle: 'Sub-floor discount exploits halted',
    ...
    },
    {
      ...
      subtitle: 'Verified Razorpay webhook payments',
    ...
    {
      title: 'AP2 Mandates',
      subtitle: 'Cryptographic DIT tokens verified',
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((stat, i) => {
        ...
        return (
          <div
            key={i}
            className="p-4 rounded-xl bg-[#121216] border border-zinc-800/90 hover:border-zinc-700 transition flex flex-col justify-between"
          >
