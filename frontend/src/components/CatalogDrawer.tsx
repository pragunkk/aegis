...
export const CatalogDrawer: React.FC<CatalogDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl rounded-2xl bg-[#121216] border border-zinc-800 p-6 shadow-2xl space-y-5">
        {/* Modal Header */}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-indigo-500 transition"
          />
        </div>
        ...
        <div className="max-h-80 overflow-y-auto space-y-2.5 pr-1">
          {filteredProducts.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="p-3.5 rounded-xl bg-zinc-900/60 border border-zinc-800/80 hover:border-zinc-700/80 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="space-y-0.5 max-w-sm">
                  ...
                  <div className="text-[10px] text-zinc-500 font-mono">Stock: {p.stock} units</div>
                </div>

                <div className="flex items-center gap-5 shrink-0 text-xs">
                  <div>
                    <div className="text-[10px] text-zinc-500">MRP</div>
                    <div className="font-semibold text-zinc-200">₹{p.mrp.toLocaleString('en-IN')}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-emerald-400 font-medium">Floor Price</div>
                    <div className="font-semibold text-emerald-400">₹{floor.toLocaleString('en-IN')}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[10px] text-zinc-500">Max Discount</div>
